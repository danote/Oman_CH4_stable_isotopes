...
```{r linker-primer-seq-as-in-meta-map, warning=FALSE, message=FALSE}
FWD <- "AA"
REV <- "AA"
```

```{r linker-primer-seq-as-in-meta-map-plus-emp, warning=FALSE, message=FALSE}
# FWD <- "AAGTGYCAGCMGCCGCGGTAA"
# REV <- "AAGGACTACNVGGGTWTCTAAT"
```
...
```{r filter-and-trim}
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(145,145), trimLeft = c((str_length(FWD)), (str_length(REV))),
...
write(asv_fasta, "data_output/ASVs_Silva_Bayes_linker_trimmed.fa")

  # count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "data_output/ASVs_counts_Silva_Bayes_linker_trimmed.txt", sep="\t", quote=F)

  # tax table:
asv_tax <- taxa_Silva
row.names(asv_tax) <- sub(">", "", asv_headers)
write.table(asv_tax, "data_output/ASVs_taxonomy_Silva_Bayes_linker_trimmed.txt", sep="\t", quote=F)
```
...
write.table(tax_tab_for_mctoolsr_Silva, "data_output/tax_tab_DADA2_mctoolsr_OM18_Silva138_Bayes_linker_trimmed.txt", row.names = FALSE, sep="\t", quote=F) # write a .txt file of the taxa table
```