...
data_literature$site <- factor(data_literature$site, levels = c("Oman/UAE", "Philippines", "Mid-Cayman Rise", "Lost City", "Ashadze II", "Kidd Creek", "Sabatier reaction 70˚C to 90˚C"))

# shapes

shapes_all_sites_named <- c("Oman/UAE" = 21, "Philippines" = 0, "Mid-Cayman Rise" = 2,  "Ashadze II" = 6, "Kidd Creek" = 4, "Sabatier reaction 70˚C to 90˚C" = 3, "Lost City" = 5)
...
## Plot
```{r H2-CH4-plot, fig.width = 5, fig.height = 3.5}
...
eps_CH4_H2O_cap13CH3D_plot <-  ggplot() +
  ...
  geom_point(data = data_clumps, aes(
    ...
    shape = site,
    fill = sampling_site), alpha = 1, size = 2.5) +
...
data_clumps_lit_all_sites$site <- factor(data_clumps_lit_all_sites$site, levels = c("Oman/UAE", "Chimaera", "Philippines", "Greece", "Mid-Cayman Rise", "Lost City", "Ashadze II", "Kidd Creek", "Sabatier reaction 70˚C to 90˚C"))
```
...
adapted_young_ea_2017_shape_leg_plot <-
  ...
  # symbology styling
  scale_shape_manual(values = shapes_all_sites_named, guide = guide_legend(order = 1, label.hjust = 0), name = "sample site", na.translate = FALSE, labels = unname(latex2exp::TeX(c("Oman/UAE", "Philippines", "Mid-Cayman Rise", "Lost City", "Ashadze II", "Kidd Creek", "Sabatier reaction 70$\\,\\degree$C to 90$\\,\\degree$C"))))+
...
## Plot
### Plot data
```{r alkanes-d13C-oman-wells-and-lit-plot, fig.height=5, fig.width=6, warning=FALSE}
# create plot
alkanes_d13C_oman_wells_and_lit_plot <- ggplot() +
  ...
  # Add NSHQ14 label
  annotate("label", label = "NSHQ14", x = 1.2, y = 1.4, label.size = 0.7) +
