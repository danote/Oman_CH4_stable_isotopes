...
Formatotrophic methanogenesis (FT)
$\text{HCOO}^{-}+\text{H}^++3\text{H}_2\text{(aq)}=\text{CH}_4\text{(aq)}+2\text{H}_2\text{O}\text{(l)}$

Formate disproportiation (FD)
...
# Formatotrophic methanogenesis (FT)
dG_r_standard_FT_kJ_per_mol <- -210.741
...
# Formatotrophic methanogenesis (FT)
thermo_data <- thermo_data %>% mutate(dG_r_FT_kJ_per_mol = (dG_r_standard_FT_kJ_per_mol * 1e3 + R * Temp * log(a_CH4_aq * a_formate^{-1} * a_H2_aq^{-3} * a_proton^{-1}))*1e-3)
...
thermo_data %>% mutate(log_a_proton = log10(a_proton), log_a_CO2_aq = log10(a_CO2_aq), log_a_formate = log10(a_formate), log_a_acetate = log10(a_acetate), log_a_CH4_aq = log10(a_CH4_aq), log_a_H2_aq = log10(a_H2_aq)) %>% 
  select(log_a_proton, log_a_CO2_aq, log_a_formate, log_a_acetate, log_a_CH4_aq, log_a_H2_aq, dG_r_HT_kJ_per_mol, dG_r_AC_kJ_per_mol, dG_r_FT_kJ_per_mol, dG_r_FD_kJ_per_mol) %>% 
  kable(digits = 1)
```
