...
---
---
title: "Methane and short-chain alkane C isotope data processing for samples obtained in Oman in 2018"
subtitle: "Source file: OM18_CH4_C2-C6.Rmd"
...
---
# Oman 2018 C$_1$-C$_6$ alkane $\delta^{13}$C - Introduction

C$_1$-C$_6$ alkane $\delta^{13}$C was measured by GC-C-IRMS as described in the methods section of the manuscript. The following code steps through the data processing and calibration to convert measured values to values vs. the international reference standard (Vienna Pee Dee Belemnite, VPDB). The gas samples processed in this document were collected in Oman in the February-March 2018 field season, except for sample OM17_14C_BS2. OM17_14C_BS2 was previously analyzed with a different chromatographic method. The re-analyzis here yielded results that were compatible results with the prior analysis.
...
iso_files_raw <-
  file.path(
    "data_raw/OM18_CH4_C2-C6_data", "OM18_CH4_C2-C6_data.cf.rds"
  ) %>%
...
iso_files <- iso_files_raw %>%
  ...
  iso_mutate_file_info(
    ...
    id1 = ifelse(file_id == "1144__NSHQ14_BS1A_0075.dxf", "NSHQ14_BS1A", id1),
...
Display an example chromatogram for CH$_4$ with mapped peaks.
```{r chroms-C1-mapped}
# make plot
chroms_C1_mapped <- iso_files %>%
  iso_filter_files(str_detect(id1, "188") == TRUE & str_detect(`GC Method`, "C2") == FALSE) %>%
  iso_plot_continuous_flow_data(
    # select data and aesthetics
    data = c(44),
    color = id1,
...
Display chromatograms of samples for C$_2$-C$_6$ alkane analyses with mapped peaks.
```{r chroms-C2-C6-mapped}
# plot the chromatograms
chroms_C2_C6_mapped <- iso_files %>%
    # select C2-C6 alkane analyses
  iso_filter_files(str_detect(id1, "NSHQ14") == TRUE & str_detect(`GC Method`, "C2") == TRUE) %>%
  iso_plot_continuous_flow_data(
    # select data and aesthetics
    data = c(44),
    color = file_id,
    # zoom in on time interval
    time_interval = c(1250, 4100),
    # provide our peak table with ids
    peak_table = peak_table_w_ids,
    # define peak labels, this can be any valid expression
    peak_label = iso_format(id = peak_info),
...
Standards are in small amplitude range, but separate tests showed that there is an acceptably small trend of $\delta^{13}$C vs. peak amplitude over this range of analyses.
...
```{r omit-OM17-14C-BS2_from-summary}
# note: this sample was a re-run of a prior measurement with different chromatographic conditions. The earlier measurement yielded a d13C of ethane (-5.98 per mil vpdb) within a per mil of this analyis (-5.22 per mil vpdb). The earlier measurement had higher peak amplitude (0.83 V) than this measurement (0.46 V), so I omit the latter measurement from the final peak data summary and use the former measurement for publication.
peak_data_summary <- peak_data_summary %>% filter(id1 != "OM17_14C_BS2")
```
...
peak_table_calibrated %>%
  iso_export_calibration_to_excel(
    filepath = format(Sys.Date(), "data_output/%Y%m%d_OM18_CH4_C2-C6_data_calibrated.xlsx"),
