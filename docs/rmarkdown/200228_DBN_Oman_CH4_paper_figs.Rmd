...
data <- read.xlsx("data_raw/Summary_metadata/Oman_Geochem_2012-2018_6.xlsx") # read in data from excel file
...
data_literature <- read.xlsx("data_raw/Summary_metadata/CH4_isotope_comparison_data_Nothaft_et_al_5.xlsx")
...
data_water_isotopes <- data %>% filter(!is.na(H2O_dD_permil_VSMOW)) %>% select(well, year_sampled, H2O_dD_permil_VSMOW, H2O_d18O_permil_VSMOW)
...
data_water_isotopes_summ_well_year <- data_water_isotopes %>% group_by(well, year_sampled) %>% summarize(n = n(), H2O_dD_permil_VSMOW_mean_well_year = mean(H2O_dD_permil_VSMOW), H2O_dD_permil_VSMOW_sd = sd(H2O_dD_permil_VSMOW), H2O_d18O_permil_VSMOW_mean_well_year = mean(H2O_d18O_permil_VSMOW), H2O_d18O_permil_VSMOW_sd = sd(H2O_d18O_permil_VSMOW))
...
data_water_isotopes_summ_well <- data_water_isotopes_summ_well_year  %>% group_by(well) %>% summarize(n = n(), H2O_dD_permil_VSMOW_mean_well = mean(H2O_dD_permil_VSMOW_mean_well_year), H2O_dD_permil_VSMOW_sd = sd(H2O_dD_permil_VSMOW_mean_well_year), H2O_d18O_permil_VSMOW_mean_well = mean(H2O_d18O_permil_VSMOW_mean_well_year), H2O_d18O_permil_VSMOW_sd = sd(H2O_d18O_permil_VSMOW_mean_well_year))
...
data_CD <- data %>% select(well, year_sampled, depth_sampled_mbtc, water_type, CH4_d13C_permil_VPDB_LBNL, CH4_dD_permil_VSMOW_LBNL,
...
# print
data_CD %>% select(well, year_sampled, CH4_d13C_permil_VPDB_interlab_mean, CH4_dD_permil_VSMOW_interlab_mean)
```

Summarize WAB188 $\delta^{13}\text{C}_{\text{CH}_4}$ data
```{r summ-WAB188}
# make data frame of relevant data
data_WAB188 <- data %>% filter(well == "WAB188") %>%  filter(!is.na(CH4_d13C_permil_VPDB_LBNL) | !is.na(CH4_d13C_permil_VPDB_CUB)) %>% select(year_sampled, CH4_d13C_permil_VPDB_LBNL,
...
# make data frame of relevant data
data_WAB188_DIC <- data %>% filter(well == "WAB188") %>%  filter(DIC_was_measured == TRUE) %>% select(year_sampled, DIC_uM)
...
# make data frame of relevant data
data_WAB71 <- data %>% filter(well == "WAB71") %>%  filter(!is.na(CH4_d13C_permil_VPDB_LBNL) | !is.na(CH4_d13C_permil_VPDB_CUB)) %>% select(year_sampled, CH4_d13C_permil_VPDB_LBNL,
CH4_d13C_permil_VPDB_CUB)
...
# make data frame of relevant data
data_CM2A <- data %>% filter(well == "CM2A") %>% select(CH4_d13C_permil_VPDB_CUB, CH4_d13C_permil_VPDB_MIT, CH4_d13C_permil_VPDB_UCLA)
...
data_CD_well_year_summ <- data_CD %>% group_by(well,year_sampled) %>% summarise(CH4_d13C_permil_VPDB_interlab_mean = mean(CH4_d13C_permil_VPDB_interlab_mean), CH4_dD_permil_VSMOW_interlab_mean = mean(CH4_dD_permil_VSMOW_interlab_mean), water_type = first(water_type))
...
data_CH4_H2 <- data %>% select(well, year_sampled, water_type, depth_sampled_mbtc,  water_level_mbtc, H2_uM, H2_was_measured, H2_unc_uM, H2_LQ_uM, CH4_uM, CH4_was_measured, CH4_unc_uM, CH4_LQ_uM)
...
data_CH4_H2 <- data_CH4_H2 %>% group_by(well, year_sampled) %>% summarise(water_type = first(water_type), H2_uM_LQ_sub_for_NA = mean(H2_uM_LQ_sub_for_NA), CH4_uM_LQ_sub_for_NA = mean(CH4_uM_LQ_sub_for_NA), H2_LQ_uM = mean(H2_LQ_uM), CH4_LQ_uM = mean(CH4_LQ_uM), H2_uM = mean(H2_uM), CH4_uM = mean(CH4_uM))
...
H2_CH4_plot <- data_CH4_H2 %>% ggplot() +
  ...
  geom_text(data = data_CH4_H2 %>% filter(is.na(CH4_uM)), aes(
    ...
    label = "^",
    color = water_type
    ), alpha = 0.7, angle = 180, nudge_y = -0.1, nudge_x = 0.01, show.legend = FALSE) +
  ...
  geom_text(data = data_CH4_H2 %>% filter(is.na(H2_uM)), aes(
    ...
    label = "^",
    color = water_type
    ), alpha = 0.7, angle = 90, nudge_x = -0.1, nudge_y = 0.0015, show.legend = FALSE) +
  ...
  # text labels
  geom_text_repel(alpha = 1, size=3.5, force = 5, segment.color = "black", segment.alpha = .2, show.legend = FALSE) +

  # axis styling
  scale_y_continuous(trans = 'log10',
      breaks = trans_breaks('log10', function(x) 10 ^ x, n=6),
      labels = trans_format('log10', math_format(10^.x)),
      limits = c(7e-3, 1e3), expand = c(0, 0),
...
data_C_gathered <- data_CD %>% select(well, year_sampled, water_type, CH4_d13C_permil_VPDB_CUB, CH4_d13C_permil_VPDB_MIT, CH4_d13C_permil_VPDB_UCLA, CH4_d13C_permil_VPDB_LBNL, site, depth_sampled_mbtc) %>% gather(key = "parameter", value = "CH4_d13C_permil_VPDB", -well, -water_type, -year_sampled, -site, -depth_sampled_mbtc)
...
data_D_gathered <- data_CD %>% select(well, water_type, year_sampled, CH4_dD_permil_VSMOW_CUB, CH4_dD_permil_VSMOW_MIT, CH4_dD_permil_VSMOW_UCLA, CH4_dD_permil_VSMOW_LBNL) %>% gather(key = "parameter", value = "CH4_dD_permil_VSMOW", -well, -water_type, -year_sampled)
...
data_CD_gathered$well <- factor(data_CD_gathered$well, levels = c("NSHQ04", "WAB71", "CM2A", "NSHQ14"))
...
data_C1_C2_C3 <- data %>% filter(!is.na(C2H6_uM)) %>%  select(well, year_sampled, depth_sampled_mbtc, water_type, CH4_uM, CH4_was_measured, CH4_LQ_uM, CH4_unc_uM, C2H6_uM, C2H6_was_measured, C2H6_LQ_uM, C2H6_unc_uM, C3H8_uM, C3H8_was_measured, C3H8_LQ_uM, C3H8_unc_uM,
...
data_C1_C2_C3 <- data_C1_C2_C3 %>% group_by(well, year_sampled) %>% summarise(water_type = first(water_type), C1_over_C2_plus_C3 = mean(C1_over_C2_plus_C3),  CH4_d13C_permil_VPDB_CUB = mean(CH4_d13C_permil_VPDB_CUB, na.rm = TRUE), CH4_uM = mean(CH4_uM), C2H6_uM = mean(C2H6_uM), C3H8_uM = mean(C3H8_uM)) %>% ungroup()
...
data_clumps <- data_clumps %>% left_join(data_water_isotopes_summ_well %>% select(well, H2O_dD_permil_VSMOW_mean_well))
...
data_clumps_gathered_cap13CH3D <- data_clumps %>% select(well, year_sampled, cap13CH3D_permil_MIT, cap13CH3D_permil_UCLA) %>% gather(-well, -year_sampled, value = cap13CH3D_permil, key = lab)
...
# summarize
data_clumps_gathered_summ_well_year_cap13CH3D <- data_clumps_gathered_cap13CH3D %>% select(well, year_sampled, cap13CH3D_permil) %>%  group_by(well, year_sampled) %>% summarise(n = n(), cap13CH3D_permil_mean = mean(cap13CH3D_permil), cap13CH3D_permil_sd = sd(cap13CH3D_permil))
...
data_clumps_gathered_summ_well_interannual_cap13CH3D <- data_clumps_gathered_summ_well_year_cap13CH3D %>% group_by(well) %>% summarise(n=n(), cap13CH3D_permil_mean_mean = mean(cap13CH3D_permil_mean), cap13CH3D_permil_mean_sd = sd(cap13CH3D_permil_mean))
...
data_12CH2D2 <- data %>% filter(!is.na(cap12CH2D2_permil_UCLA)) %>%  select(well, cap13CH3D_permil_UCLA, cap13CH3D_unc_permil_UCLA, cap12CH2D2_permil_UCLA, cap12CH2D2_unc_permil_UCLA, site)
...
data_alkanes_d13C <- data %>% filter(!is.na(C2H6_d13C_permil_VPDB)) %>% select(well, year_sampled, depth_sampled_mbtc, CH4_d13C_permil_VPDB_CUB, CH4_d13C_unc_permil_CUB, CH4_d13C_permil_VPDB_MIT, CH4_d13C_unc_permil_MIT, C2H6_d13C_permil_VPDB, C2H6_d13C_unc_permil, C3H8_d13C_permil_VPDB, C3H8_d13C_unc_permil)
...
data_alkanes_d13C_gathered <- data_alkanes_d13C  %>% select(-depth_sampled_mbtc) %>% gather("compound_lab", "d13C_permil_VPDB", -well, -year_sampled, -CH4_d13C_unc_permil_CUB, -CH4_d13C_unc_permil_MIT, -C2H6_d13C_unc_permil, -C3H8_d13C_unc_permil)
...
data_alkanes_d13C_gathered <- data_alkanes_d13C_gathered %>% select(well, year_sampled, C_atoms, compound_lab, d13C_permil_VPDB, uncertainty_d13C_permil)
